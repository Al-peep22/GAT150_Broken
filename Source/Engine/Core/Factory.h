...
namespace viper {
	...
	public:
		PrototypeCreator(std::unique_ptr<T> proto) : prototype(std::move(proto)) {}
		std::unique_ptr<Object> Create() override {
	...
	{
		return viper::Factory::Instance().Create<T>(name);
	}
	...
	{
		auto instance = viper::Factory::Instance().Create<T>(name);
